...
"""
方法一：内置函数
执行用时：36 ms, 在所有 Python3 提交中击败了76.29%的用户
内存消耗：14.8 MB, 在所有 Python3 提交中击败了11.51%的用户
...
a ^ b 得到了一个无进位加法结果，如果要得到 a + b 的最终值，我们还要找到进位的数，
把这二者相加。在位运算中，我们可以使用与操作获得进位：
...
执行用时：40 ms, 在所有 Python3 提交中击败了47.98%的用户
内存消耗：14.8 MB, 在所有 Python3 提交中击败了13.43%的用户
"""
class Solution:
    def getSum(self, a: int, b: int) -> int:
        # 2^32
        ...
        while b != 0:
            # 与运算计算进位
            carry = (a & b) << 1
            # 异或运算取余范围限制在[0, 2^32-1]范围内
            a = (a ^ b) % MASK
            b = carry % MASK

        """
        因为在python中int不是32位的，输出是64位，所以一个负数比如-2, 64位表示就是0x00000000FFFFFFFE, 
        用python求取这个16进制的值int('0x00000000FFFFFFFE', 16), 得到的数字是4294967294 不是我们想要的-2，
        所以： a^mask是先对a的前32位取反，对应-2，就得到0x0000000000000002 再用～操作符对所以位置取反，对应-2，
        得到0xFFFFFFFFFFFFFFFE
